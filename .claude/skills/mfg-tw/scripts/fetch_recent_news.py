...
def fetch_google_news(query: str, days: int, max_items: int, timeout: int) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--target-name", required=True, help="Company name, for example 台積電 or TSMC.")
    parser.add_argument("--target-ticker", default=None, help="Optional ticker, for example 2330.TW.")
...
def main() -> int:
    ...
    if args.source in {"google", "both"}:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
            futures = [
                pool.submit(fetch_google_news, query, args.days, args.max_per_query, args.timeout)
                for query in queries
            ]
